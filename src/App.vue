  ...
  <div :class="['web-rtc', { 'rtc-mobile': isMobileView }]">
    ...
    <transition name="play">
      <div class="play" v-if="room">
        <div class="play-room_name" v-if="isMobileView">
          {{ room.explain.name }}
        </div>
        <Play :streams="streams"> </Play>
        <div
          class="mobile-chat_entry"
          @click="showMobileChat"
          v-if="isMobileView && peersLength"
        >
...
interface RTCManagerLike {
  ...
  dcData: {
    on: (
      event: string,
      handler: (...args: never[]) => void,
    ) => RTCManagerLike["dcData"];
    off: (
      event: string,
      handler?: (...args: never[]) => void,
    ) => RTCManagerLike["dcData"] | false;
    emit: (
      event: string,
      data: unknown,
  ...
  dcFile: {
    on: (
      event: string,
      handler: (...args: never[]) => void,
    ) => RTCManagerLike["dcFile"];
    off: (
      event: string,
      handler?: (...args: never[]) => void,
    ) => RTCManagerLike["dcFile"] | false;
    emit: (
      event: string,
      data: File | Blob,
      desc?: Record<string, unknown>,
...
const slefVideoBtnStatus = reactive({
  audio: false,
  desktopShare: false,
  video: false,
  cameraSwitch: false,
...
rtcManager
  .on("streams", handleStreamChange)
  .on("peers:change", handlePeersChange);
